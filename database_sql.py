    ...
    def return_matches(self, hashes):
        ...
        with self.cursor() as cur:
            for split_values in grouper(values, 1000):
                ...
                new_split_values=copy.deepcopy(split_values)
                l=len(list(new_split_values))
                placeholders = ', '.join(['UNHEX(%s)'] * l)
                query = self.SELECT_MULTIPLE.replace('(%s)', '({})').format(placeholders)
                cur.execute(query, list(split_values))
